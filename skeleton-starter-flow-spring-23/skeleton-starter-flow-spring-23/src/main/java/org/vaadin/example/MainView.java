...
public class MainView extends VerticalLayout {
    ...
    public MainView(@Autowired PokemonService service) {
        ...
        Grid<Datos> grid = new Grid<>(Datos.class, true);
        grid.addColumn(Datos::getName).setHeader("Nombre");
        grid.addColumn(Datos::getAttack).setHeader("Ataque");
        grid.addColumn(Datos::getDefense).setHeader("Defensa");
        grid.addColumn(Datos::getTipo1).setHeader("Tipo1");
        grid.addColumn(Datos::getSpeedDefense).setHeader("Tipo2");
        inputs.add(comboBox);
        Button boton1 = new Button("Lee caracter", e -> {
